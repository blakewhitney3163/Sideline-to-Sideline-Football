...
// Estimated fair market value — used as default offer in FA signing
function fairMarketValue(pos: string, ovr: number, devTrait: string = 'Normal'): number {
  ...
  return Math.round(base * (traitMul[devTrait] ?? 1.0) * 10) / 10;
}
...
export default function Franchise({ userTeam, currentSeason }: Props) {
  ...
  const handleSign = async () => {
    ...
    if (!result.success) {
      alert(result.reason ?? 'Could not sign player.');
      setWorking(false);
      return;
    }
    setSigningId(null);
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 11, color: '#444' }}>
            ...
            {rosterSpots && (
              <span style={{ marginLeft: 'auto' }}>
                {rosterSpots.active}/53 active · {rosterSpots.ps}/16 PS
              </span>
            )}
          </div>
        </div>
                ...
                <div key={contract.id} style={{ borderBottom: '1px solid #111', background: isExtending || isReleasing ? '#0f0f0f' : 'transparent' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 100px 90px 160px 100px 120px', gap: 8, padding: '8px 12px', alignItems: 'center' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        {trait.short && (
                          <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: trait.color + '22', color: trait.color, fontWeight: 'bold', letterSpacing: 1 }}>{trait.short}</span>
                        )}
                        {grade && (
                          <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: grade.color + '22', color: grade.color, fontWeight: 'bold', letterSpacing: 1 }}>{grade.label}</span>
                        )}
                      </div>
                  ...
                  {isExtending && currentExtend && (
                    <div style={{ margin: '0 12px 12px', padding: '14px 18px', background: '#0a1a0a', border: '1px solid #1a3a1a', borderRadius: 6 }}>
                      ...
                      {newAvailable < 0 && (
                        <div style={{ marginTop: 10, fontSize: 11, color: '#e57373' }}>Over cap by {fmtSalary(Math.abs(newAvailable))} — reduce salary or cut a player first.</div>
                      )}
                    </div>
                  )}
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 14 }}>
            {POSITIONS.map(pos => (
              <button key={pos} onClick={() => { setFaPos(pos); setSigningId(null); }} style={{
                padding: '3px 9px',
                background: faPos === pos ? '#4FC3F7' : '#141414',
                border: `1px solid ${faPos === pos ? '#4FC3F7' : '#222'}`,
                borderRadius: 3, color: faPos === pos ? '#000' : '#555',
                fontSize: 11, cursor: 'pointer', fontWeight: faPos === pos ? 'bold' : 'normal',
              }}>{pos}</button>
            ))}
          </div>

          {/* Roster / cap context bar */}
          {rosterSpots && cap && (
                ...
                <div key={fa.id} style={{ borderBottom: '1px solid #111', background: isSigning ? '#0a0f1a' : 'transparent' }}>
                  ...
                  {isSigning && signingPlayer && (
                    <div style={{ margin: '0 12px 12px', padding: '14px 18px', background: '#07101a', border: '1px solid #1a3a5a', borderRadius: 6 }}>
                      <div style={{ fontSize: 10, color: '#4FC3F7', letterSpacing: 2, marginBottom: 12 }}>
                        OFFER CONTRACT — {fa.first_name} {fa.last_name}
                      </div>
                      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                        <div>
                          <div style={{ fontSize: 10, color: '#444', marginBottom: 6 }}>YEARS</div>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: '#444', marginTop: 4 }}>
                            Market: {fmtSalary(fairMarketValue(fa.position, fa.overall_rating, fa.dev_trait))}/yr
                          </div>
                        </div>
                        <div>
                          <div style={{ fontSize: 10, color: '#444', marginBottom: 6 }}>CAP AFTER SIGNING</div>
                          <div style={{ fontSize: 13, color: signCapLeft < 0 ? '#e57373' : '#4caf50' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                          <button onClick={handleSign} disabled={working || signCapLeft < 0} style={{
                            padding: '8px 20px', borderRadius: 4, fontSize: 12, cursor: signCapLeft < 0 ? 'not-allowed' : 'pointer', fontWeight: 'bold',
