  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        <h2 style={{ color: T.textPrimary, fontSize: 18, fontWeight: 700, margin: 0 }}>{viewSeason} Standings</h2>
      ...
      {view === 'division' ? (
        <>
            ...
            {(['AFC', 'NFC'] as const).map(conf => (
              <div key={conf}>
                ...
                {DIVISION_ORDER.map(div => {
                  ...
                  return <DivisionBlock key={div} division={div} teams={divTeams} playoffIds={playoffIds} userTeamId={userTeamId} />;
                })}
              </div>
            ))}
          </div>
          <div style={{ marginTop: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              <PlayoffSeedPanel conf="AFC" seeds={afcSeeds} userTeamId={userTeamId} />
              <PlayoffSeedPanel conf="NFC" seeds={nfcSeeds} userTeamId={userTeamId} />
            </div>
                  ...
                  <thead>
                    <tr>
                      {['', 'TEAM', 'W', 'L', 'PCT', 'DIV'].map(h => (
                        <th key={h} style={{ color: T.textDim, fontSize: 9, fontWeight: 700, padding: '4px 6px', textAlign: 'left', borderBottom: `1px solid ${T.borderFaint}` }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {confTeams.map((team, i) => {
                      ...
                      return (
                        <React.Fragment key={team.id}>
                          {isCutline && (
                            <tr><td colSpan={6} style={{ padding: '4px 6px', color: '#e57373', fontSize: 9, borderTop: `1px dashed #e57373` }}>— playoff cutline (after seed 7) —</td></tr>
                          )}
                          <tr style={{ background: isUser ? T.bgBlue : 'transparent' }}>
                            <td style={{ padding: '6px', color: isDivWinner ? '#FF8740' : '#4FC3F7', fontSize: 10, width: 20 }}>
                              {seedNum > 0 && <span>{seedNum}</span>}
                            </td>
                            <td style={{ padding: '6px', color: isUser ? '#4FC3F7' : T.textPrimary, fontWeight: isUser ? 700 : 400 }}>
                              {team.city} {team.name}
                              {isUser && <span style={{ color: '#4FC3F7', marginLeft: 4 }}>◆</span>}
                            </td>
                            <td style={{ padding: '6px', color: T.textMuted }}>{team.wins}</td>
                            <td style={{ padding: '6px', color: T.textMuted }}>{team.losses}</td>
                            <td style={{ padding: '6px', color: T.textMuted }}>{pct(team.wins, team.losses)}</td>
                            <td style={{ padding: '6px', color: T.textDim, fontSize: 11 }}>{team.division}</td>
                          </tr>
                        </React.Fragment>
                      );
                    })}
