...
export function getLeagueStats(season: number): {
  ...
  const base = `FROM stats st JOIN players p ON st.player_id = p.id JOIN teams t ON st.team_id = t.id JOIN games g ON st.game_id = g.id WHERE g.season = ? AND g.is_simulated = 1`;
...
export function getTeamSeasonStats(season: number): any[] {
  const pointRows = db.prepare(`
    ...
    WHERE g.season = ? AND g.is_simulated = 1 AND g.is_playoff = 0 GROUP BY t.id
  `).all(season) as any[];
  ...
  const statRows = db.prepare(`
    ...
    WHERE g.season = ? AND g.is_simulated = 1 AND g.is_playoff = 0 GROUP BY s.team_id
  `).all(season) as any[];
