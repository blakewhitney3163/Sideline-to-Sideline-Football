...
export function calcFairMarket(ovr: number, position: string, devTrait: string): number {
  const group = POS_RATE_GROUP[position] ?? position;
  const rates = MARKET_RATE_TABLE[group] ?? MARKET_RATE_TABLE['LB'];
...
function batchGetTeamTypes(teamIds: number[], season: number): Map<number, string> {
  ...
  const rosterRows = db.prepare(`
    ...
    FROM players WHERE team_id IN (${ph}) AND roster_status = 'active'
    GROUP BY team_id
  `).all(...teamIds) as any[];
    ...
    for (const player of players) {
      ...
      if (player.overall_rating >= 76) {
        logNewsEvent({
          ...
          teamId: team.id, playerId: player.id, season,
        });
      }
    }
  }
...
export function cpuResignAttempts(userTeamId: number): { attempted: number; resigned: number; hitFA: number } {
  ...
  for (const team of cpuTeams) {
    ...
    for (const player of expiring) {
      ...
      if (Math.random() < acceptChance) {
        contractRepo.update(player.id, years, offer, Math.round(offer * years * (guaranteedPct / 100) * 10) / 10, guaranteedPct);
        capLeft -= offer;
        resigned++;
        if (isNotable) {
          logNewsEvent({
        ...
        if (isNotable) {
          logNewsEvent({
            eventType: 'fa-departure', category: 'transactions',
...
export function cpuFABiddingEngine(userTeamId: number): { totalSigned: number; teamsActive: number } {
  ...
  const freeAgents = db.prepare(`
    ...
    FROM players WHERE is_free_agent = 1 AND roster_status = 'free_agent'
  ...
  for (const fa of freeAgents) {
    ...
    for (const { teamId, bid } of bidders) {
      ...
      db.transaction(() => {
        ...
        contractRepo.create(fa.id, teamId, years, bid, Math.round(bid * years * (guaranteedPct / 100) * 10) / 10, guaranteedPct);
      })();
...
export function applyFranchiseTag(
  ...
  if (existing) return { success: false, reason: `${tagLabel} already used this offseason.` };
  const player = db.prepare(`
    SELECT p.id, p.first_name, p.last_name, p.position, p.dev_trait,
           p.overall_rating, p.team_id, COALESCE(c.annual_salary, 0) as annual_salary
...
export function getOffseasonStatus(teamId: number | null): {
  ...
  draftGenerated: boolean; draftComplete: boolean; faOpen: boolean;
} {
  const season = getCurrentSeason();
  ...
  const draftComplete = draftGenerated
    ...
    : false;
  const faOpen = !!(db.prepare('SELECT value FROM settings WHERE key = ?').get(`fa_open_${season}`) as any);
  return {
